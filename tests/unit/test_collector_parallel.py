...
from policyradar.collector import RateLimiter, collect_sources
from policyradar.exceptions import NetworkError, SourceError
...
def test_parallel_collection_reduces_runtime() -> None:
    ...
    with (
        patch("policyradar.collector._collect_single", side_effect=delayed_collect),
        patch("policyradar.collector.get_circuit_breaker_manager", return_value=manager),
...
def test_parallel_collection_isolates_source_errors() -> None:
    ...
    with (
        patch("policyradar.collector._collect_single", side_effect=selective_collect),
        patch("policyradar.collector.get_circuit_breaker_manager", return_value=manager),
        patch.dict(os.environ, {"RADAR_MAX_WORKERS": "5"}, clear=False),
    ):
        articles, errors = collect_sources(sources, category="test", min_interval_per_host=0.0)
...
def test_max_workers_one_preserves_sequential_order() -> None:
    ...
    with (
        patch("policyradar.collector._collect_single", side_effect=ordered_collect),
        patch("policyradar.collector.get_circuit_breaker_manager", return_value=manager),
        patch.dict(os.environ, {"RADAR_MAX_WORKERS": "5"}, clear=False),
    ):
...
def test_env_var_radar_max_workers_is_used() -> None:
    ...
    with (
        patch("policyradar.collector._collect_single", return_value=[]),
        patch("policyradar.collector.get_circuit_breaker_manager", return_value=manager),
        patch("policyradar.collector.ThreadPoolExecutor") as mock_executor,
