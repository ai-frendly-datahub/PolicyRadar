...
def handle_change_detect(*, db_path: Path, days: int = 14, limit: int = 10) -> str:
    """Detect policy or terms changes from recent article analysis."""
    cutoff = datetime.now() - timedelta(days=days)
    conn = duckdb.connect(str(db_path), read_only=True)
    try:
        rows = conn.execute(
            """
            SELECT title, source, entities_json, collected_at
            FROM articles
            WHERE collected_at >= ?
            ORDER BY collected_at DESC
            LIMIT ?
    ...
    changes: list[str] = []
    for title, source, entities_json, collected_at in rows:
        title_lower = str(title).lower()
        if any(kw in title_lower for kw in change_keywords):
            entity_info = ""
            if entities_json:
                try:
                    entities = cast(dict[str, object], json.loads(str(entities_json)))
                    entity_names = list(entities.keys())
                    if entity_names:
                        entity_info = f" [{', '.join(entity_names)}]"
                except json.JSONDecodeError:
                    pass
            changes.append(f"- {title}{entity_info} ({source}, {collected_at})")

    if not changes:
        return "No policy or terms changes detected in the recent period."
