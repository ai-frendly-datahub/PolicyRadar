...
from policyradar.mcp_server.tools import (
    handle_change_detect,
...
def _list_tool_specs() -> list[dict[str, object]]:
    return [
        ...
        {
            "name": "change_detect",
            "description": "Detect policy, terms of service, or regulation changes from recent articles.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "days": {"type": "integer", "minimum": 1},
...
def _call_tool_handler(name: str, arguments: object) -> str:
    ...
    if name == "change_detect":
        return handle_change_detect(
            db_path=_db_path(),
            days=_as_int(args.get("days"), 14),
            limit=_as_int(args.get("limit"), 10),
        )
